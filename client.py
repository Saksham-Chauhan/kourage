...
async def on_react(bot, ctx, user, ticket_channel, _logger):
    ...
    client_name_embed=discord.Embed(title="",description="Please enter your name",colour=0x11806a)
    ...
    email_embed=discord.Embed(title="",description="Please enter your email-id",colour=0x11806a)
    ...
    phone_embed=discord.Embed(title="",description="Please enter your phone no.",colour=0x11806a)
    ...
    project_name_embed=discord.Embed(title="",description="What is your Project Name ?",colour=0x11806a)
    project=await ctx.send(embed=project_name_embed,delete_after=60)
    project_name = await embeds.ctx_input(ctx, bot, project, user = user)
    if not project_name:
        await welcome.delete()
        raise Exception("'project_name' timed out.")
    logger.info("'project_name' input successful.")

    dueDate_embed=discord.Embed(title="Please enter the date when you want this project",description="enter in format DD-MM-YY ",colour=0x11806a)
    dueDate=await ctx.send(embed=dueDate_embed,delete_after=60)
    client_dueDate = await embeds.ctx_input(ctx, bot, dueDate, user = user)
    if not client_dueDate:
        await welcome.delete()
        raise Exception("'client_dueDate' timed out.")
    logger.info("'client_dueDate' input successful.")

    budget_embed=discord.Embed(title="",description="Please enter your budget",colour=0x11806a)
    budget=await ctx.send(embed=budget_embed,delete_after=60)
    client_budget = await embeds.ctx_input(ctx, bot, budget, user = user)
    if not client_budget:
        await welcome.delete()
        raise Exception("'client_budget' timed out.")
    logger.info("'client_budget' input successful.")
    ...
    sendEmbed.add_field(name='Project Due date (expected):', value = str(client_dueDate)+"\n\n\n", inline=False)
    sendEmbed.add_field(name='Project Budget:', value = "Rs. "+str(client_budget)+"\n\n\n", inline=False)
    ...
    if not category:
        category = guild.create_category("Client")
    ...
    await new_client_channel.send("Please wait while. The management team will get back to you soon.")
