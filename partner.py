...
async def on_react(bot, ctx, user, ticket_channel, _logger):
    global logger
    logger = _logger
    logger.info('~on_ready:client called for ' + str(user.id))
    ...
    partner_name_embed=discord.Embed(title="",description="Please enter your name",colour=0x11806a)
    ...
    organisation_name_embed=discord.Embed(title="",description="What is your Organisation Name ?",colour=0x11806a)
    ...
    reason_embed=discord.Embed(title="",description="Please enter why you want to be a partner",colour=0x11806a)
    ...
    overwrites = {
        ...
        discord.utils.get(guild.roles, name = 'partner_' + str(unique_id)): discord.PermissionOverwrite(read_messages=True),
    ...
    await new_partner_channel.send("Please wait while. The management team will get back to you soon.")
