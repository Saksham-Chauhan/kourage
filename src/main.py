...
async def show_issues(ctx):
    ...
    due_embed = discord.Embed(title="Enter the s.no of the project of which you want to see issues",
                              description="Please just give a number only eg: 1", colour=0x11806a)
...
async def add_user(ctx):
    ...
    due_embed = discord.Embed(title="Enter the s.no of the project in which you want to add members",
                              description="Please just give a number only eg: 1", colour=0x11806a)
    message = await ctx.send(embed=due_embed, delete_after=70)
    project_number = await embeds.ctx_input(ctx, bot, message)
    if not project_number:
...
async def remove_user(ctx):
    ...
    due_embed = discord.Embed(title="Enter the s.no of the project of which you want to remove members",
                              description="Please just give a number only eg: 1", colour=0x11806a)
    message = await ctx.send(embed=due_embed, delete_after=70)
    project_number = await embeds.ctx_input(ctx, bot, message)
