    ...
    for uid in ulist:
        ...
        try:
            ...
            if str(preq) == "<Response [422]>":
                ...
                member_add_embed = discord.Embed(
                    title="Member already added to project: " + project_name + "\n User ID: " + str(uid),
                    description=preq, colour=0x11806a)
                ...
                member_add_embed = discord.Embed(
                    title="Member added to project: " + project_name + "\nUser ID: " + str(uid), description=preq,
                    colour=0x11806a)
                await ctx.send(embed=member_add_embed, delete_after=15)
                await ctx.message.delete()
        except Exception as e:
        ...
        if not i in udict:
            ...
            try:
                ...
                if (str(ecode) == "<Response [200]>"):
                    ...
                    member_add_embed = discord.Embed(
                        title="Removed user from project: " + project_name + "\nUser ID: " + str(i), description=ecode,
                        colour=0x11806a)
                    await ctx.send(embed=member_add_embed, delete_after=15)
                    await ctx.message.delete()
                else:
        ...
        for i in issues["issues"]:
            ...
            if not due:
                ...
                if (delta.days < 0):
                    ...
                    if "assigned_to" not in i:
                        list = list + ("Issue #" + str(i["id"]) + " ( Expires in " + str(
